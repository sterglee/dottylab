...
public class JavaUtilities {
    ...
    // of course that .jar should be on the classpath
      static public URL  jarPathOfClass(String className) {
        try {
    ...
    JavaGlobals.jarFilePath = jarPathOfClass("scalaSci.RichDouble2DArray").toString().replace("file:/", "/");
              JavaGlobals.compFile = jarPathOfClass("scala.tools.nsc.interpreter.Imain").toString().replace("file:/", "/");
   ...
   System.out.println("libFile = "+JavaGlobals.libFile);
    JavaGlobals.reflectFile = jarPathOfClass("scala.reflect.Enum").toString().replace("file:/","/");
