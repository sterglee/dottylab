...
// display the LaTex formula graphically
    ...
    public static boolean getVerbose() {
        return true;
                //scalaSci.PrintFormatParams.getVerbose();
     }
...
public static Object eval(String expr)  {
    var   res = scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(expr);
    return res; 
}
...
public static String getFile() {
    javax.swing.JFileChooser chooser = new javax.swing.JFileChooser(""); // SOSSOSDirectory.Current().get().path());
...
public static String appendClasspaths(Vector <String> newPaths) {
   ...
   // construct and initialize a new Scala Interpreter with appended classpath that includes the 'newPath'if (GlobalValues.interpreterTypeForPane == GlobalValues.EJMLMat)
...
return classpath();
   ...
   public static void exec(String fileName)  {
        ...
        if (fileName.indexOf(File.separatorChar)==-1)  // not absolute path: append current directory
           fileName = ""; // SOSSOS Directory.Current().get().path()+File.separator+fileName;
