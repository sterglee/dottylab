...
import static scalalab.JavaGlobals.ARPACKFile;
...
public class scalaLab extends JFrame implements WindowListener, ActionListener {
    ...
    public void closeGUI() {
        ...
        if (userOption == JOptionPane.YES_OPTION) {
            ...
            File outPropFile = new File( "scalalab.props");
            try {
    ...
    *
    ...
    public scalaLab(String ScalaSciClassPath, String[] additionalToolboxes) {
        ...
        if (GlobalValues.scala3Engine==null ) {
            GlobalValues.scala3Engine = new javax.script.ScriptEngineManager().getEngineByName("scala");
            if (GlobalValues.scala3Engine == null) {
                JOptionPane.showMessageDialog(null, "Cannot create Scala 3 engine");
            }
        }
      //  if (GlobalValues.dottyEngine==null ) {
       //     GlobalValues.dottyEngine = new dotty.tools.repl.ScriptEngine();
       //     if (GlobalValues.dottyEngine == null) {
        //        JOptionPane.showMessageDialog(null, "Cannot create Dotty engine");
       //     }
     //   }
       // GlobalValues.scala3Engine.createBindings();
    //            JOptionPane.showMessageDialog(null, "Scala engine: "+GlobalValues.scala3Engine.toString());
     //   JOptionPane.showMessageDialog(null, "Dotty engine: "+GlobalValues.dottyEngine.toString());
     //   try {
        //        GlobalValues.dottyEngine.eval("var x=9999;");
...
/*
try {
    System.out.println("eval = " + GlobalValues.scala3Engine.eval("var ff=99; println(ff)"));
}
...
//System.out.println("before SysUtils.ConsoleWindow");
        GlobalValues.consoleOutputWindow = new SysUtils.ConsoleWindow();

  //      System.out.println("after SysUtils.ConsoleWindow");
...
// create Undocked
        ...
        SwingUtilities.invokeLater(new Runnable() {
         ...
         //           GlobalValues.scalalabMainFrame.scalalabConsole.interpreterWithAppendedCP(GlobalValues.ScalaSciClassPathComponents);
           ...
           //         System.out.println("\n\n Displayed Toolboxes information cleared. You can reopen toolbox windows as required \n\n");
                ...
                //    JOptionPane.showMessageDialog(null, "scanning: "+GlobalValues.jarFilePath);
                    Vector scalaSciPlottingClasses = scalaExec.ClassLoaders.JarClassLoader.scanLib(GlobalValues.jarFilePath, "scalaSci");
    ...
    public static void detectJarLibPaths(String watchStr) {
     ...
     //  JOptionPane.showMessageDialog(null, "Full Path of the main ScalaLab .jar file  = " + fullJarPath);
...
//JOptionPane.showMessageDialog(null, "scalalabPath = "+GlobalValues.jarFilePath+ " scalalablibpath = "+GlobalValues.scalalabLibPath);
    }


    static public void mkJavaClassPath() {


        if (GlobalValues.hostIsUnix) {
            String compFile = scalalab.JavaUtilities.jarPathOfClass("dotty.tools.dotc.Main").toString().replace("file:/", "");
            //if (compFile==null)
            //   JOptionPane.showMessageDialog(null, "compFile null");

            String libFile = scalalab.JavaUtilities.jarPathOfClass("scala.runtime.Arrays").toString().replace("file:/", "");
            //  if (libFile==null)
            //          JOptionPane.showMessageDialog(null, "libFile null");


            // reflectFile = scalalab.JavaUtilities.jarPathOfClass("scala.reflect.api.Names").toString().replace("file:/","/");
            String swingFile = scalalab.JavaUtilities.jarPathOfClass("scala.swing.Swing").toString().replace("file:/", "/");


            String jfreechartFile = scalalab.JavaUtilities.jarPathOfClass("org.jfree.chart.ChartFactory").toString().replace("file:/", "/");
            //      if (jfreechartFile==null)
            //          JOptionPane.showMessageDialog(null, "jfreechartfile null");

            //   System.out.println(jfreechartFile);
            String jsciFile = scalalab.JavaUtilities.jarPathOfClass("JSci.maths.wavelet.Cascades").toString().replace("file:/", "/");
            //   System.out.println(jsciFile);

            String javacppFile = scalalab.JavaUtilities.jarPathOfClass("org.bytedeco.javacpp.DoublePointer").toString().replace("file:/", "/");
            String openblasFile = scalalab.JavaUtilities.jarPathOfClass("org.bytedeco.javacpp.presets.openblas").toString().replace("file:/", "/");
            String mtjColtSGTFile = scalalab.JavaUtilities.jarPathOfClass("no.uib.cipr.matrix.AbstractMatrix").toString().replace("file:/", "/");
            String ApacheCommonsFile = scalalab.JavaUtilities.jarPathOfClass("org.apache.commons.math3.ode.nonstiff.ThreeEighthesIntegrator").toString().replace("file:/", "/");
            String ejmlFile = scalalab.JavaUtilities.jarPathOfClass("org.ejml.ops.NormOps").toString().replace("file:/", "/");
            String rsyntaxTextAreaFile = scalalab.JavaUtilities.jarPathOfClass("org.fife.ui.rsyntaxtextarea.RSyntaxTextArea").toString().replace("file:/", "/");
            String matlabScilabFile = scalalab.JavaUtilities.jarPathOfClass("matlabcontrol.MatlabConnector").toString().replace("file:/", "/");
            String jblasFile = scalalab.JavaUtilities.jarPathOfClass("org.jblas.NativeBlas").toString().replace("file:/", "/");
            String numalFile = scalalab.JavaUtilities.jarPathOfClass("numal.Linear_algebra").toString().replace("file:/", "/");

            String JASFile = scalalab.JavaUtilities.jarPathOfClass("org.matheclipse.core.eval.EvalEngine").toString().replace("file:/", "/");
            String LAPACKFile = scalalab.JavaUtilities.jarPathOfClass("org.netlib.lapack.LAPACK").toString().replace("file:/", "/");

            String ARPACKFile = scalalab.JavaUtilities.jarPathOfClass("org.netlib.lapack.Dgels").toString().replace("file:/", "/");


            StringBuilder scalaSettings = new StringBuilder();

            //   scalaSettings.classpath().setByUser_$eq(true);

            scalaSettings.append("ScalaLab.jar" + ":");
            scalaSettings.append(compFile + ":");
            //   scalaSettings.classpath().add(reflectFile);
            scalaSettings.append(libFile + ":");
            scalaSettings.append(swingFile + ":");

            if (GlobalValues.hostIsLinux64 || GlobalValues.hostIsWin64 || GlobalValues.hostIsMac) {
                scalaSettings.append(javacppFile + ":");
                scalaSettings.append(openblasFile + ":");
            }

            scalaSettings.append(jfreechartFile + ":");
            scalaSettings.append(jsciFile + ":");
            scalaSettings.append(mtjColtSGTFile + ":");
            scalaSettings.append(ApacheCommonsFile + ":");
            scalaSettings.append(ejmlFile + ":");
            scalaSettings.append(rsyntaxTextAreaFile + ":");
            scalaSettings.append(matlabScilabFile + ":");
            scalaSettings.append(jblasFile + ":");
            scalaSettings.append(numalFile + ":");
            scalaSettings.append(JASFile + ":");
            scalaSettings.append(LAPACKFile + ":");
            scalaSettings.append(ARPACKFile);


            System.setProperty("java.class.path", scalaSettings.toString());
            ...
            String swingFile = scalalab.JavaUtilities.jarPathOfClass("scala.swing.Swing").toString().replace("file:/", "");
            String ejmlFile = scalalab.JavaUtilities.jarPathOfClass("org.ejml.ops.NormOps").toString().replace("file:/", "/");
            jfreechartFile = scalalab.JavaUtilities.jarPathOfClass("org.jfree.chart.ChartFactory").toString().replace("file:/", "");
            String jsciFile = scalalab.JavaUtilities.jarPathOfClass("JSci.maths.wavelet.Cascades").toString().replace("file:/", "");
            String javacppFile = scalalab.JavaUtilities.jarPathOfClass("org.bytedeco.javacpp.DoublePointer").toString().replace("file:/", "");
            String openblasFile = scalalab.JavaUtilities.jarPathOfClass("org.bytedeco.javacpp.presets.openblas").toString().replace("file:/", "");
    ...
    public static void main(String[] args) {
     ...
     //   System.out.println("starting ScalaLab");
     ...
     //   System.out.println("before decoding "+watchURL);

        String watchStr = watchURL.toString();
        watchStr = decodeFileName(watchStr);

     //   System.out.println("before detectJarLibPaths");

        detectJarLibPaths(watchStr);


        // mkJavaClassPath();
        String  extraJarsForScalaClasspathFolder = GlobalValues.scalalabLibPath.replace("lib", "defaultToolboxes");
        StringBuilder sb = new StringBuilder();
        File [] toolboxesFolderFiles = (new java.io.File(extraJarsForScalaClasspathFolder)).listFiles();  // get the list of files at the default toolboxes folder
        if (toolboxesFolderFiles!=null) {  // extraJarsForKotlinClasspath folder not empty
            int numFiles = toolboxesFolderFiles.length;
            for (int f = 0; f < numFiles; f++) {
                String currentFileName = toolboxesFolderFiles[f].getName();

                if (currentFileName.endsWith(".jar")) {

                    if (GlobalValues.hostIsUnix)
                        sb.append("defaultToolboxes/"+currentFileName + ":");
                    else   // Windows
                        sb.append("defaultToolboxes\\"+currentFileName + ";");
                }
            }
    ...
    /*    if (GlobalValues.hostIsUnix) {
           ...
           String myJavaClassPath = "ScalaLab.jar:libScala3/compiler-interface-1.3.5.jar:libScala3/dist_3-3.1.0.jar:libScala3/scala-library-2.13.6.jar:" +
                    ...
                    "lib/jhall.jar:lib/jna-5.3.1.jar:lib/jSciJPlasmaJSparseJTransforms.jar:lib/jsearch.jar:lib/jsyntaxpane-1.1.5.jar:lib/LBFGS.jar:lib/matlabscilab.jar:" +
                    "lib/MTJColtSGTJCUDA.jar:lib/netlib-java-0.9.3.jar:lib/NumericalRecipesNUMAL.jar:lib/objenesis-1.2.jar:lib/optimization.jar:lib/orsonpdf-1.7.jar:lib/PDFRenderer.jar:" +
                    "lib/RSyntaxTextArea.jar:lib/txt2xhtml.jar:lib/xmlgraphics-commons.jar:extralib/javacpp.jar:extralib/openblas-platform.jar:extralib/openblas.jar";
...
*/
        ...
        try {
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                GlobalValues.scalalabMainFrame.scalalabConsole.createInterpreterForResetFastJBLAS();
    ...
    class StandAloneApplicationActionScala extends AbstractAction {
        ...
        public void transformScriptToStandAlone() {
            ...
            File scriptToRunFile = new File( GlobalValues.scriptObjectName + runScriptExtension);
            try {
