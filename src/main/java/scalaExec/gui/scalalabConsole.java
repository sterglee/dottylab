...
public class scalalabConsole  extends Console  {
  ...
  //  static  scala.tools.nsc.interpreter.Results.Result  rs;
   ...
   @Override
    ...
    //            interpreterWithaddedCP(GlobalValues.ScalaSciClassPathComponents);
            }
        });
  ...
  clearConsoleItem.addActionListener(new ActionListener() {
     ...
     public void mkPaths() {
//JOptionPane.showMessageDialog(null, "in makePaths");
         String jarFile = GlobalValues.jarFilePath;
       //  JOptionPane.showMessageDialog(null, "jarfile = "+jarFile);
         JavaGlobals.jarFilePath = jarFile;

         if (GlobalValues.hostIsUnix) {
             compFile = jarPathOfClass("dotty.tools.dotc.Main").toString().replace("file:/", "");
         //if (compFile==null)
          //   JOptionPane.showMessageDialog(null, "compFile null");

             libFile =  jarPathOfClass("scala.runtime.Arrays").toString().replace("file:/","");
           //  if (libFile==null)
  ...
  ejmlFile = jarPathOfClass("org.ejml.ops.NormOps").toString().replace("file:/", "/");
  ...
  // reflectFile = jarPathOfClass("scala.reflect.api.Names").toString().replace("file:/","");
   swingFile = jarPathOfClass("scala.swing.Swing").toString().replace("file:/", "");
   ejmlFile = jarPathOfClass("org.ejml.ops.NormOps").toString().replace("file:/", "/");jfreechartFile =jarPathOfClass("org.jfree.chart.ChartFactory").toString().replace("file:/", "");
   jsciFile = jarPathOfClass("JSci.maths.wavelet.Cascades").toString().replace("file:/", "");
   javacppFile = jarPathOfClass("org.bytedeco.javacpp.DoublePointer").toString().replace("file:/", "");
   openblasFile = jarPathOfClass("org.bytedeco.javacpp.presets.openblas").toString().replace("file:/", "");
   ...
   ARPACKFile = jarPathOfClass("org.netlib.lapack.Dgels").toString().replace("file:/",  "");
      ...
      //   scalaSettings.classpath().setByUser_$eq(true);
         ...
         scalaSettings.classpath().add(jarFile);
         scalaSettings.classpath().add(compFile);
      //   scalaSettings.classpath().add(reflectFile);
       scalaSettings.classpath().add(libFile);
        scalaSettings.classpath().add(swingFile);
  
         if (GlobalValues.hostIsLinux64 || GlobalValues.hostIsWin64 || GlobalValues.hostIsMac) {
          scalaSettings.classpath().add(javacppFile);
          scalaSettings.classpath().add(openblasFile);
         }

         scalaSettings.classpath().add(jfreechartFile);
         scalaSettings.classpath().add(jsciFile);
         scalaSettings.classpath().add(mtjColtSGTFile);
         scalaSettings.classpath().add(ApacheCommonsFile);
         scalaSettings.classpath().add(ejmlFile);
         scalaSettings.classpath().add(rsyntaxTextAreaFile);
         scalaSettings.classpath().add(matlabScilabFile);
         scalaSettings.classpath().add(jblasFile);
         scalaSettings.classpath().add(numalFile);
         scalaSettings.classpath().add(JASFile);
         scalaSettings.classpath().add(LAPACKFile);
         scalaSettings.classpath().add(ARPACKFile);
      ...
      public void createInterpreterEJMLorJAMA()  {
          ...
          Vector <String> classpath = GlobalValues.ScalaSciClassPathComponents;
           for (int k=0; k<classpath.size(); k++) {
              ...
              scalaSettings.classpath().add(clsp.trim());  
          }
...
/** Create a new interpreter. */
      ...
      // add a single path specification to the classpath
  public void  interpreterWithaddedCP(String classpath) {
  ...
  // add an array of classpath specifications to the classpath
  public void  interpreterWithaddedCP(String [] classpath) {
      GlobalValues.interpreterTypeForPane = GlobalValues.JAMAMat;
          GlobalValues.interpreterClassPathComponents= null; 
 ...
 scalaExec.Interpreter.GlobalValues.globalInterpreterPane.installScalaCompletion();
  ...
  // add a Vector of classpath specifications to the classpath
  public void  interpreterWithaddedCP(final Vector classpath) {
  ...
  // add a Vector of classpath specifications to the classpath
  public void  interpreterWithaddedCPEJML(Vector classpath) {
    ...
    { 
        ...
        inputString = theLine.trim();
                 ...
                 Runnable  r = new Runnable() {
                    ...
                    public void run() {
               var   rs =  scalaExec.Interpreter.GlobalValues.globalInterpreter.interpret(inputString);
