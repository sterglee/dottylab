...
"//  Press F12 to close all displayed figures \n\n";
  ...
  def appendBasicPathsToSettings() = {
    
    //  append to the classpath the main ScalaLab libraries
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.jarFilePath)  // the Scalalab .jar file by itself
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.compFile)  // Scala Compiler
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.libFile)     // Scala Libraries
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.reflectFile)  // scala-reflect file
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.swingFile)  // Scala Swing

    
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.jfreechartFile)   // ScalaLab JFreeChart library
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.numalFile)  // NUMAL library
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.mtjColtSGTFile)  // MTJ, Colt, SGT libraries
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.ApacheCommonsFile)   // Apache Common Maths current version file
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.ejmlFile)   // EJML file
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.rsyntaxTextAreaFile)  // RSyntaxTextArea file
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.matlabScilabFile)  // MATLAB - SciLab connection file
      
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.jblasFile)  // JBLAS File
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.jsciFile)   // jSci  file
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.javacppFile)
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.JASFile)     // Java Algebra System
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.LAPACKFile)    // LAPACK linear algebra 
      scala.tools.nsc.Settings.classpath.add(scalalab.JavaGlobals.ARPACKFile)  // ARPACK linear algebra 
  ...
  def init {
    ...
    var settings = new scala.tools.nsc.Settings()   // settings for the main Scala Interpreter
         ...
         /*
          ...
          var   classpathScalaSci = GlobalValues.ScalaSciClassPathComponents
           for (k <- 0 until  classpathScalaSci.size) {
              ...
              scala.tools.nsc.Settings.classpath.add(clsp.trim())
          ...
          if (toolboxesFolderFiles!=null)
            for (file <- toolboxesFolderFiles if file.getName.endsWith(".jar")) {
               ...
               if (GlobalValues.ScalaSciClassPathComponents.contains(toolboxName)==false) {
                 ...
                 scala.tools.nsc.Settings.classpath.add(toolboxName)
      ...
      (new SwingWorker[ Unit, Unit ] {
        ...
        //   if (scalaExec.Interpreter.GlobalValues.compilerOptimizationFlag == true)
         ...
         //    settings.optimise.value = false
           ...
           scalaExec.Interpreter.GlobalValues.globalInterpreter = new scala.tools.nsc.interpreter.IMain( ){
    ...
    amap.put( "clearConsole",  new AbstractAction {
        ...
        imap.put(KeyStroke.getKeyStroke( KeyEvent.VK_F9, 0), "executeScala3")
            ...
            def actionPerformed(e:ActionEvent) {
              ...
              var res= GlobalValues.scala3Engine.eval(currentText)
               println("evaluating with Scala 3 "+currentText+" res = "+res)
